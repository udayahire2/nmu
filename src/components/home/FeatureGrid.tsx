...
const features = [
    {
        ...
        color: "text-blue-500",
        bg: "bg-blue-500/10",
        gradient: "from-blue-500/5", // Explicit class for Tailwind
    ...
    },
    {
        ...
        bg: "bg-purple-500/10",
        gradient: "from-purple-500/5", // Explicit class for Tailwind
...
export function FeatureGrid() {
    return (
        <section className="container mx-auto py-24 md:py-32 px-4" aria-labelledby="features-heading">
            <div className="text-center mb-20">
                <h2 id="features-heading" className="text-3xl font-bold tracking-tight md:text-5xl mb-6">Resources</h2>
                ...
                {features.map((feature, index) => (
                    ...
                    >
                        <Card className="group relative overflow-hidden h-full border-border/40 bg-background/40 backdrop-blur-sm transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:-translate-y-2 dark:hover:shadow-[0_8px_30px_rgb(0,0,0,0.3)] hover:border-primary/20">
                            {/* Fixed: Use transparent to transparent but with gradient color start */}
                            <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br ${feature.gradient} to-transparent pointer-events-none`} />
                            ...
                            <CardHeader className="relative z-10">
                                <div className={`w-16 h-16 rounded-2xl ${feature.bg} flex items-center justify-center mb-6 transition-transform duration-500 shadow-inner ring-1 ring-inset ring-black/5`}>
                                    <svg
                                        ...
                                        aria-hidden="true" // QA Fix: Decorative icon
                                    >
                                        <motion.path
                                            d={feature.path}
                                            variants={{
                                                rest: { pathLength: 1, opacity: 1, transition: { duration: 0.5 } },
                                                hover: { pathLength: 1.1, opacity: 1, transition: { duration: 1, repeat: Infinity, repeatType: "mirror" } }
                                            }}
                                        />
                                        <motion.path
                                            d={feature.path}
                                ...
                                <CardTitle className="flex items-center gap-2 text-2xl mb-2">
                                    {feature.title}
                                    <ArrowUpRight className="h-5 w-5 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-muted-foreground" aria-hidden="true" />
                                </CardTitle>
                            </CardHeader>
